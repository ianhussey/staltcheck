...
write_rds(statcheck_on_apa_dataset, "../data/statcheck_on_apa_dataset_new.rds")
#statcheck_on_apa_dataset <- read_rds("../data/statcheck_on_apa_dataset_new.rds")
...
possibly_t_to_d <- possibly(t_to_d, otherwise = NA)

dat_es_ttest <- statcheck_on_apa_dataset |>
  filter(test_type == "t") |>
  mutate(d = future_map2(test_value, as.numeric(df2), possibly_t_to_d)) |>
  unnest(d) |>
  rename(cohens_d = d, cohens_d_ci_lower = CI_low, cohens_d_ci_upper = CI_high) |>
  select(-CI)

write_rds(dat_es_ttest, "../data/dat_es_ttest.rds")
#dat_es_ttest <- read_rds("../data/dat_es_ttest.rds")


# possibly_z_to_d <- possibly(z_to_d, otherwise = NA)
#
# dat_es_ztest <- statcheck_on_apa_dataset |>
#   filter(test_type == "Z") |>
#   mutate(d = future_map2(test_value, as.numeric(df2), possibly_z_to_d)) |> # is df2 == n? second argument is n
#   unnest(d) |>
#   rename(cohens_d = d, cohens_d_ci_lower = CI_low, cohens_d_ci_upper = CI_high) |>
#   select(-CI)
# 
# write_rds(dat_es_ztest, "../data/dat_es_Ztest.rds")
# #dat_es_ztest <- read_rds("../data/dat_es_Ztest.rds")
...
ggplot(dat_es_ttest, aes(cohens_d)) +
  geom_histogram(binwidth = 0.1, boundary = 0) +
  theme_linedraw()

ggplot(dat_es_ttest, aes(abs(cohens_d))) +
  geom_histogram(binwidth = 0.1, boundary = 0) +
  theme_linedraw()
...
dat_es_ttest |>
  summarize(median_abs_d = median(abs(cohens_d), na.rm = TRUE))



temp <- dat_es_ttest |>
  filter(abs(cohens_d) > 5 & computed_p < 10e-10)


ggplot(dat_es_ttest, aes(log(abs(cohens_d)), log(computed_p))) +
  geom_point() +
  theme_linedraw() 

ggplot(dat_es_ttest, aes(abs(cohens_d), log(computed_p))) +
  geom_point() +
  theme_linedraw() +
  coord_cartesian(xlim = c(0, 10))

ggplot(dat_es_ttest, aes(log(abs(cohens_d)), log(computed_p)*-1)) +
...
temp <- dat_es_ttest |>
  filter(as.numeric(df2) < 30 & 
           computed_p < 10e-20)


temp <- dat_es_ttest |>
  mutate(df2 = as.numeric(df2)) |>
  filter(abs(cohens_d) < 5.1 & abs(cohens_d) > 4.9)
